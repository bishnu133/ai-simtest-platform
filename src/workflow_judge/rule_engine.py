"""
Rule Engine v3 — Deterministic hard checks with semantic topic support.

V3 additions over v2:
- Async evaluate_async() for semantic topic checks
- Optional LLM evaluator reference for semantic mode
- Sync evaluate() preserved for backward compatibility
- topic_eval_mode awareness on topic rules
"""
from __future__ import annotations
import re
import logging
from typing import TYPE_CHECKING, List, Optional
from .models import HardRule, HardRuleResult, HardRuleType, MatchType, TopicEvalMode, TurnViolation
...
class RuleEngine:
    def __init__(self, rules: List[HardRule], llm_evaluator: Optional["LLMWorkflowEvaluator"] = None):
        self.rules = rules
        self.llm_evaluator = llm_evaluator  # V3: for semantic topic checks

    def evaluate(self, bot_messages: List[str], all_messages: List[str] | None = None,
                 turn_count: int = 0) -> List[HardRuleResult]:
        """Synchronous evaluate — uses keyword mode for all topic rules."""
        all_messages = all_messages or bot_messages
        return [self._evaluate_rule(rule, bot_messages, all_messages, turn_count)
                for rule in self.rules]

    async def evaluate_async(self, bot_messages: List[str],
                              all_messages: List[str] | None = None,
                              turn_count: int = 0) -> List[HardRuleResult]:
        """V3: Async evaluate — supports semantic topic checks via LLM."""
        all_messages = all_messages or bot_messages
        results = []
        for rule in self.rules:
            if (rule.rule_type in (HardRuleType.FORBIDDEN_TOPIC, HardRuleType.REQUIRED_TOPIC)
                    and rule.topic_eval_mode_enum == TopicEvalMode.SEMANTIC
                    and self.llm_evaluator is not None):
                result = await self._check_topic_semantic(rule, bot_messages)
            else:
                result = self._evaluate_rule(rule, bot_messages, all_messages, turn_count)
    ...
    def _evaluate_rule(self, rule, bot_messages, all_messages, turn_count) -> HardRuleResult:
        ...
        return HardRuleResult(rule_id=rule.id, rule_name=rule.name, passed=True,
                              severity=rule.severity, evidence="Unknown rule type",
    ...
    def _check_forbidden_phrase(self, rule, bot_messages, all_messages, turn_count) -> HardRuleResult:
        ...
        return HardRuleResult(rule_id=rule.id, rule_name=rule.name, passed=passed,
                              severity=rule.severity, evidence=evidence,
                              rule_type=HardRuleType.FORBIDDEN_PHRASE.value,
    ...
    def _check_required_phrase(self, rule, bot_messages, all_messages, turn_count) -> HardRuleResult:
        ...
        return HardRuleResult(rule_id=rule.id, rule_name=rule.name, passed=passed,
                              severity=rule.severity, evidence=evidence,
                              rule_type=HardRuleType.REQUIRED_PHRASE.value)
    ...
    def _check_forbidden_topic(self, rule, bot_messages, all_messages, turn_count) -> HardRuleResult:
        ...
        return HardRuleResult(rule_id=rule.id, rule_name=rule.name, passed=passed,
                              severity=rule.severity, evidence=evidence,
                              rule_type=HardRuleType.FORBIDDEN_TOPIC.value)
    ...
    def _check_required_topic(self, rule, bot_messages, all_messages, turn_count) -> HardRuleResult:
        ...
        return HardRuleResult(rule_id=rule.id, rule_name=rule.name, passed=passed,
                              severity=rule.severity, evidence=evidence,
                              rule_type=HardRuleType.REQUIRED_TOPIC.value)

    # V3: Semantic topic checking via LLM
    async def _check_topic_semantic(self, rule: HardRule, bot_messages: List[str]) -> HardRuleResult:
        """Use LLM to semantically evaluate topic presence."""
        phrases = self._get_phrases(rule)
        combined = " ".join(bot_messages)
        is_forbidden = rule.rule_type == HardRuleType.FORBIDDEN_TOPIC

        detected, confidence, reasoning = await self.llm_evaluator.evaluate_topic_semantic(
            combined, phrases, is_forbidden)

        if is_forbidden:
            passed = not detected
            evidence = f"Semantic: topic {'detected' if detected else 'not detected'} (confidence: {confidence:.2f}). {reasoning}"
        else:
            passed = detected
            evidence = f"Semantic: topic {'found' if detected else 'not found'} (confidence: {confidence:.2f}). {reasoning}"
    ...
    def _check_max_turns(self, rule, turn_count) -> HardRuleResult:
        ...
        return HardRuleResult(rule_id=rule.id, rule_name=rule.name, passed=passed,
                              ...
                              evidence=f"Conversation took {turn_count} turns (max: {max_turns})",
                              rule_type=HardRuleType.MAX_TURNS_TO_RESOLVE.value)

    def _check_must_escalate(self, rule, bot_messages) -> HardRuleResult:
        indicators = ["human agent", "live agent", "customer support",
                      "speak to a representative", "transfer you", "connect you",
                      "real person", "human representative", "escalate",
                      "support team", "customer service representative"]
        ...
        return HardRuleResult(rule_id=rule.id, rule_name=rule.name, passed=len(found) > 0,
                              severity=rule.severity,
    ...
    def _check_must_not_escalate(self, rule, bot_messages) -> HardRuleResult:
        indicators = ["human agent", "live agent", "transfer you", "connect you",
                      "speak to a representative", "real person"]
        custom = self._get_phrases(rule)
        all_ind = indicators + [i.lower() for i in custom]
        combined = " ".join(bot_messages).lower()
        found = [i for i in all_ind if i in combined]
